...
usage: ptyhelp run [flags] command args...
  ...
  -kill-after string
    	grace period after timeout before forced termination (e.g. 5s)
...
usage: ptyhelp patch [flags] command args...
  ...
  -check
    	exit 1 when the target file or -o output would change (CI staleness check)
  -cols uint
    	PTY width (setting this flag implies PTY capture; cannot combine with -pty=false) (default 256)
  -combined
    	merge stderr into stdout (same as -stderr=merge)
  -dry-run
    	print the patched file to stdout when it would change; write neither it nor -o
  -fence string
